...
@JsonSubTypes({
        @JsonSubTypes.Type(value = PartieNotFoundErrorResponse.class)
})
@AllArgsConstructor
@SuperBuilder(toBuilder = true)
@Data
public abstract class PartieErrorResponse {
    @Schema(description = "endpoint appelé", example = "/partie/{codePartie}")
    private final String uri;
    @Schema(description = "code http de la réponse", example = "404")
    private final HttpStatus httpStatus;
    @Schema(description = "code de l'erreur", example="PARTIE_IS_NOT_FOUND")
    private final ErrorCode errorCode;
    @Schema(description = "message d'erreur")
    private final String errorMessage;

    protected PartieErrorResponse(final ErrorCode errorCode){
